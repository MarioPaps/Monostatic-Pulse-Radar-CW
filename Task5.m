%Third scan-targets 1,2  present
...
[~,angle_est]= maxk(max_storer,2); %create a function to find k maxima
angle_est= angle_est+29;

%vectors to store estimated target parameters
techo=zeros(2,1);
R_estimated=zeros(2,1);
RCS_estimated= zeros(2,1);
peak= zeros(2,1);
...
%compute threshold
threshold=specifyThreshold(Rxout,5);
